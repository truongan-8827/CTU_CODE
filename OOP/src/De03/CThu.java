...
public class CThu extends CNguoi {
	...
	public void nhap() {
		...
		System.out.println("Nhap so ao:"); so=sc.nextInt(); sc.nextLine();
		System.out.println("Nhap vi tri:"); vtri=sc.nextLine();
		System.out.println("Nhap so ban thang:"); bthang=sc.nextLong(); sc.nextLine();
		System.out.println("Nhap mua giai:"); mua=sc.nextLine();
		System.out.println("Nhap cau lac bo:"); clbo=sc.nextLine();
	}
	@Override
	public void in() {
		super.in();
		System.out.println("-So ao: "+so+", vi tri: "+vtri+", so ban thang: "+bthang+", mua giai: "+mua+", cau lac bo: "+clbo);
	}
	public long tinhLuong() {
		long t=0;
		if(vtri.equals("thu mon")) t=(10000000+bthang*500000);
		else if(vtri.equals("hau ve")) t=(11000000+bthang*500000);
		else if(vtri.equals("trung ve")) t=(11500000+bthang*500000);
		else if(vtri.equals("tien ve")) t=(12000000+bthang*500000);
		else if(vtri.equals("tien dao")) t=(14000000+bthang*500000);
		return (long)(0.9*t);
	}
	...
	public static void main(String[] args) {
		...
		for(int i=0;i<n;i++) {
			if(ds[i].getTenLop().equals("CThu")) xetCauThu[i]=true;
			else xetCauThu[i]=false;
		}
		// co the su dung (doi tuong)instanceof(ten class) tra ve true hoac false, thay vi boolean
			...
			if(xetCauThu[i]) {
				...
				for(int j=i;j<n;j++) {
					if(ds[j].getMua().equals(ds[i].getMua()) && ds[j].getCLB().equals(ds[i].getCLB())) {
						...
						xetCauThu[j]=false;
						if(tuoi2>=18 && tuoi2<=24) bthang1+=ds[j].getBThang();
						else if(tuoi2>=25 && tuoi2<=28) bthang2+=ds[j].getBThang();
						else if(tuoi2>28) bthang3+=ds[j].getBThang();
					}
				}
